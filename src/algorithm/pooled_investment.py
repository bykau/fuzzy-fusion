'''
The implementation of PooledInvestment algorithm from
...
def get_belief(data, trustw_list, s_obs_count):
    ...
    for obj_index in data.keys():
        ...
        for term_ind, term in enumerate(term_list[:]):
            term_list[term_ind] = (term**g)*term/denom
...
def pooled_investment(data=None, gt=None, accuracy_truth=None, s_number=None):
    ...
    while trustw_delta > eps and iter_number < max_rounds:
        ...
        trustw_list = get_trustw(data=data, belief=belief, trustw_prev=trustw_prev, s_obs_count=s_obs_count)
